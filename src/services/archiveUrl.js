...
export function pathFor(link) {
  ...
  // A collection opens on all its films, unless it is wholly one genre (Film_Noir opens that pill)
  if (link.type === 'collection') return `/browse?collection=${encodeURIComponent(link.id)}${VIDEO_CATEGORIES.some(c => c.id === link.id && c.wholly) ? '' : '&genre=all'}${link.newest ? '&sort=publicdate+desc' : ''}`;
