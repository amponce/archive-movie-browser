...
const index = {
  peewee: { i: 1, t: "Pee-wee's Big Adventure", y: 1985, p: '/a.jpg', v: 7.0, k: 900, c: 0.95, g: ['Comedy', 'Adventure'], l: 91 },
  reanimator_dvd: { i: 2, t: 'Re-Animator', y: 1985, p: '/b.jpg', v: 7.2, k: 1400, c: 0.9, g: ['Horror', 'Comedy'], l: 86 },
  reanimator_rip: { i: 2, t: 'Re-Animator', y: 1985, p: '/b.jpg', v: 7.2, k: 1400, c: 0.97, g: ['Horror', 'Comedy'], l: 86 },
  detour: { i: 3, t: 'Detour', y: 1945, p: '/c.jpg', v: 7.2, k: 426, c: 1, g: ['Crime', 'Drama'], l: 68 },
  chaplin_short: { i: 4, t: 'The Rink', y: 1916, p: '/d.jpg', v: 6.9, k: 120, c: 1, g: ['Comedy'], l: 24 },
  no_length: { i: 5, t: 'Unknown Comedy', y: 1950, p: '/e.jpg', v: 5, k: 10, c: 0.9, g: ['Comedy'] },
  no_poster: { n: 1, c: 0.9 },
};
...
test('indexFilms gives every film of the genre, one upload per film, the surest one', () => {
  const films = indexFilms(index, { genre: 'Comedy' });
  assert.deepEqual(films.map(f => f.identifier).sort(), ['chaplin_short', 'no_length', 'peewee', 'reanimator_rip']);
  const reanimator = films.find(f => f.title === 'Re-Animator');
  assert.equal(reanimator.identifier, 'reanimator_rip');
...
test('indexFilms filters by decade and by length like the Archive.org list', () => {
  assert.deepEqual(indexFilms(index, { genre: 'Comedy', decade: 1980 }).map(f => f.identifier).sort(), ['peewee', 'reanimator_rip']);
  assert.deepEqual(indexFilms(index, { genre: 'Comedy', minRuntime: 40 }).map(f => f.identifier).sort(), ['no_length', 'peewee', 'reanimator_rip'], 'a short goes, an unknown length stays');
  assert.deepEqual(indexFilms(index, { genre: 'Comedy', shorts: true }).map(f => f.identifier), ['chaplin_short']);
...
test('indexFilms sorts the way the menu says', () => {
  ...
  assert.deepEqual(ids('downloads'), ['reanimator_rip', 'peewee', 'chaplin_short', 'no_length'], 'most popular is most rated');
  assert.deepEqual(ids('tmdb_rating').slice(0, 2), ['reanimator_rip', 'peewee']);
