...
// The subtitle files that go with the video playing, labelled by the language in their names,
// English first and Archive.org's machine captions last. In an upload holding several films
// (or parts), only the files named after this video; if none are, every subtitle file.
const LANGUAGES = [
  ['en', 'English', /(^|[._ -])(en|eng|english)$/i], ['es', 'Spanish', /(^|[._ -])(es|spa|spanish|espanol|español)$/i],
  ['fr', 'French', /(^|[._ -])(fr|fre|fra|french|francais|français)$/i], ['de', 'German', /(^|[._ -])(de|ger|deu|german|deutsch)$/i],
  ['it', 'Italian', /(^|[._ -])(it|ita|italian|italiano)$/i], ['pt', 'Portuguese', /(^|[._ -])(pt|por|portuguese|portugues|português|pt-br)$/i],
  ['ar', 'Arabic', /(^|[._ -])(ar|ara|arabic)$/i], ['ru', 'Russian', /(^|[._ -])(ru|rus|russian)$/i],
  ['ja', 'Japanese', /(^|[._ -])(ja|jpn|japanese)$/i], ['zh', 'Chinese', /(^|[._ -])(zh|chi|zho|chinese)$/i],
  ['nl', 'Dutch', /(^|[._ -])(nl|dut|nld|dutch)$/i], ['sv', 'Swedish', /(^|[._ -])(sv|swe|swedish)$/i],
  ['pl', 'Polish', /(^|[._ -])(pl|pol|polish)$/i], ['tr', 'Turkish', /(^|[._ -])(tr|tur|turkish)$/i],
];
const SUBTITLE = /\.(srt|vtt)$/i;
const AUTO = /[._-](asr|autogenerated)(\.|$)/i;
export function subtitleTracks(files, videoName = '') {
  const all = (files || []).filter(file => SUBTITLE.test(file.name || ''));
  const stem = stemOf(videoName.split('/').pop());
  const mine = all.filter(file => stemOf(file.name.split('/').pop()).startsWith(stem));
  const tracks = (stem && mine.length ? mine : all).map((file) => {
    const auto = AUTO.test(file.name);
    const base = file.name.replace(SUBTITLE, '').replace(AUTO, '');
    const lang = auto ? null : LANGUAGES.find(([, , pattern]) => pattern.test(base));
    return { file: file.name, lang: lang?.[0] || '', label: auto ? 'Auto captions' : lang?.[1] || 'Subtitles', auto };
  });
  const rank = t => (t.auto ? 2 : t.lang === 'en' ? 0 : 1);
  return tracks.sort((a, b) => rank(a) - rank(b));
}
const stemOf = name => String(name || '').replace(/\.ia\.mp4$/i, '').replace(/\.[^.]+$/, '');
